...
class EnhancedAnnotatedSuffixArray(base.AST):
    ...
    def _compute_childtab(self, lcptab):
        ...
        stack = [0]
        n = len(lcptab)
        childtab_up = np.zeros(n, dtype=int)  # Zeros / -1 ?
        childtab_down = np.zeros(n, dtype=int)
        for i in range(n):
    ...
    def _compute_childtab_next_l_index(self, lcptab):
        ...
        stack = [0]
        n = len(lcptab)
        childtab_next_l_index = np.zeros(n, dtype=int)  # Zeros / -1 ?
        for i in range(n):
            while lcptab[i] < lcptab[stack[-1]]:
                stack.pop()
    ...
    def _compute_anntab(self, suftab, lcptab):
        ...
        """
        n = len(suftab)
        anntab = np.zeros(n, dtype=int)  # Zeros / -1 ?
